...
Provides a ``mybot-weixin`` entry point that authenticates via QR code to the
iLink WeChat bot platform (ilinkai.weixin.qq.com) and bridges messages to the
async Orchestrator via MessageBus.

Unlike ``channels/wechat.py`` (itchat-uos, risks personal-account bans), this
channel uses the *bot* API so the bot has its own independent identity — exactly
the "independent mybot friend" model.
...
class WeixinChannel(BaseChannel):
    ...
    async def _refresh_context_tokens(self) -> bool:
        ...
        try:
            ...
            if new_tokens:
                ...
                logger.info("WeixinChannel: refreshed context tokens for {} users", len(new_tokens))
                return True
            return False
        except Exception:
            logger.opt(exception=True).warning("WeixinChannel: failed to refresh context tokens")
            return False
    ...
    async def start(self) -> None:
        logger.info("WeixinChannel: starting iLink WeChat bot")
        ...
        self._state_dir = ws_root / "weixin"
        ...
        if not self._token:
            logger.info("WeixinChannel: no saved token, starting QR login")
            try:
                ok = await self._qr_login()
            except Exception:
                logger.exception("WeixinChannel: QR login failed")
                ok = False
            if not ok:
                logger.error("WeixinChannel: login failed, channel not started")
                if self._client:
                    await self._client.aclose()
                    self._client = None
                return
        else:
            logger.info("WeixinChannel: loaded saved token")
    ...
    async def shutdown(self) -> None:
        logger.info("WeixinChannel: shutting down")
    ...
    async def send_reply(self, text: str, msg: ChannelMessage) -> None:
        ...
        if not self._client or not self._token:
            logger.warning("WeixinChannel: cannot send reply — not connected")
            return
        ...
        if not ctx_token:
            logger.error(
                "WeixinChannel: no context_token for {}, dropping reply", msg.user_id
            )
            return
        ...
        for chunk in chunks:
            try:
                ...
                logger.opt(exception=True).warning(
                    "WeixinChannel: failed to send to {}", msg.chat_id
                )
    ...
    def _load_state(state_dir: Path) -> dict[str, Any]:
        ...
        try:
            ...
            logger.opt(exception=True).warning("WeixinChannel: failed to load account.json")
            return {"token": "", "get_updates_buf": "", "context_tokens": {}, "base_url": BASE_URL}
    ...
    def _save_state(
        ...
        *,
        ...
        try:
            ...
            logger.opt(exception=True).warning("WeixinChannel: failed to save account state")
            ...
            while True:
                ...
                if status == "confirmed":
                    ...
                    if token:
                        ...
                        self._persist_state(force=True)
                        logger.info(
                            "WeixinChannel: login success bot_id={} user_id={}",
                            bot_id,
                            user_id,
                        )
                        return True
                    else:
                        logger.error("WeixinChannel: confirmed but no bot_token")
                        return False
                    ...
                    if refresh_count > MAX_QR_REFRESH_COUNT:
                        logger.warning(
                            "WeixinChannel: QR expired {} times, giving up",
                            refresh_count - 1,
                        )
                        return False
                    qrcode_id, scan_url = await self._fetch_qr_code()
    ...
    async def _poll_loop(self) -> None:
        ...
        logger.info("WeixinChannel: poll loop started")
        ...
        while self._stop_event is not None and not self._stop_event.is_set():
            ...
            try:
                ...
                if self._stop_event is not None and self._stop_event.is_set():
                    break
                logger.opt(exception=True).warning("WeixinChannel: poll error")
    ...
    async def _poll_once(self) -> None:
        ...
        if is_error:
            if errcode == ERRCODE_SESSION_EXPIRED or ret == ERRCODE_SESSION_EXPIRED:
                ...
                remaining = self._session_pause_remaining_s()
                logger.warning(
                    "WeixinChannel: session expired (errcode {}), pausing {} min",
                    errcode,
        ...
        for raw_msg in msgs:
            try:
                ...
                logger.opt(exception=True).warning("WeixinChannel: message processing error")
    ...
    async def _on_message(self, raw_msg: dict) -> None:
        ...
        logger.info(
            "WeixinChannel: inbound from={} bodyLen={}",
            msg.user_id,
            len(msg.text),
        )
    ...
    async def _consume_outbound(self) -> None:
        """Read from ``bus.outbound("weixin")`` and send replies via iLink."""
        while self._stop_event is None or not self._stop_event.is_set():
            ...
            if chat_id is None:
                logger.warning(
                    "WeixinChannel: no chat_id for session {!r}, dropping reply",
                    out.session_key,
                )
                continue
            ...
            if not ctx_token:
                logger.error(
                    "WeixinChannel: no context_token for {}, dropping reply", chat_id
                )
                continue
            ...
            for chunk in chunks:
                try:
                    ...
                    logger.opt(exception=True).warning(
                        "WeixinChannel: failed to send to {}", chat_id
                    )
...
def main() -> None:
    ...
    bot = WeixinChannel(orchestrator)
    ...
    try:
        ...
        logger.info("WeixinChannel: exited")
