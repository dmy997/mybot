"""Tests for channels/weixin.py — iLink personal WeChat bot channel."""
...
class TestWeixinParse:
    ...
    def test_text_message(self):
        ...
        msg = WeixinChannel._parse(raw, allowed_from=None)
        assert msg is not None
        assert msg.session_key == "weixin:private:@user_abc"
    ...
    def test_allowed_from_filter(self):
        ...
        msg = WeixinChannel._parse(raw, allowed_from=["@friend"])
        assert msg is None
...
class TestStatePersistence:
    ...
    def test_save_creates_dir(self, tmp_path):
        d = tmp_path / "new_weixin_state"
        WeixinChannel._save_state(d, token="tk", get_updates_buf="",
...
class TestApiHelpers:
    ...
    def chan(self, mock_orchestrator):
        c = WeixinChannel(mock_orchestrator)
        c._token = "test-token"
...
class TestSendText:
    ...
    async def test_send_reply_splits_long_message(self, chan, mock_orchestrator):
        ...
        msg = ChannelMessage(
            session_key="weixin:private:@user1",
            text="hi", user_id="@user1", chat_id="@user1",
            chat_type="private", platform="weixin",
        )
...
class TestMessageBusIntegration:
    ...
    async def test_on_message_enqueues(self, chan, mock_orchestrator):
        ...
        inbound = await chan._bus.inbound("weixin:private:@u1").get()
        assert inbound.session_key == "weixin:private:@u1"
        assert inbound.content == "hello"
        assert inbound.source == "weixin"
    ...
    async def test_on_message_stores_chat_id(self, chan):
        ...
        await chan._on_message(raw)
        assert chan._chat_ids["weixin:private:@u3"] == "@u3"
    ...
    async def test_consume_outbound_sends_reply(self, chan):
        ...
        chan._chat_ids["weixin:private:@u1"] = "@u1"
        chan._context_tokens["@u1"] = "ctx_1"
        chan._token = "tk"
        chan._stop_event = asyncio.Event()

        await chan._bus.outbound("weixin").put(OutboundMessage(
            session_key="weixin:private:@u1",
            correlation_id="weixin:private:@u1",
            msg_type="final",
    ...
    async def test_consume_outbound_no_chat_id_drops(self, chan):
        ...
        await chan._bus.outbound("weixin").put(OutboundMessage(
            session_key="weixin:private:@unknown",
            correlation_id="cid",
            msg_type="final",
            data={"content": "reply"},
        ))
...
class TestWeixinChannelLifecycle:
    def test_channel_name(self, mock_orchestrator):
        chan = WeixinChannel(mock_orchestrator)
        assert chan.channel_name == "weixin"
