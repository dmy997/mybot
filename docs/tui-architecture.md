...
├── app.py         # ChatApp 主应用（~417 行）
├── widgets.py     # 自定义 Widget 组件（~242 行）
...
使用 `set_interval` 定时器切换 `●` / `○` 字符，模拟闪烁效果。工具完成时 `mark_done()` 移除组件，失败时 `mark_error()` 显示红色错误。

**注意**：当前 TUI 中 `ToolStatus` 已定义但流式渲染实际使用 `_on_tool_exec_start` 回调直接在 `StreamingMessage` 中添加工具调用标记（`● **name**(args)`），而非单独的 `ToolStatus` widget。`ToolStatus` 保留用于未来可能的独立工具指示。
...
| 回调 | 触发时机 | 行为 |
|------|---------|------|
| `_on_delta(token)` | 每个 content token | 累积到 stream，0.15s 节流滚动到底部 |
| `_on_thinking(token)` | 每个 reasoning token | 当前忽略（未来可展开思考过程） |
| `_on_thinking_done()` | 推理完成 | 当前忽略 |
| `_on_tool_start(name, brief)` | SSE tool_call delta | 当前忽略（SSE delta 不可靠，改用 exec_start） |
| `_on_tool_exec_start(name, args, idx, total)` | 工具开始执行 | `stream.add_token("● **name**(args)")` + 强制刷新 |
| `_on_tool_exec_end(ev)` | 工具执行完成 | 错误时 `stream.add_token("✗ **detail**")` |
| `_on_new_turn()` | Agent 内部新轮次 | `stream.add_token("\n\n")` 分隔轮次 |
