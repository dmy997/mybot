...
```
  ...
  SSE consumer (cid=bbb)                               WS consumer (cid=eee)
```
...
| 流式回调 | 直接馈送到 StreamRenderer | 转为 OutboundMessage → 共享队列 → 消费者过滤 |
...
```python
# core/message_bus.py:119-120, 123, 134-137
class MessageBus:
    def __init__(self, outbound_maxsize: int = 256, inbound_maxsize: int = 64):
        self._inbound_queues: dict[str, asyncio.Queue[InboundMessage]] = {}
        self._outbound: asyncio.Queue[OutboundMessage] = asyncio.Queue(outbound_maxsize)

    def inbound(self, session_key: str) -> asyncio.Queue[InboundMessage]:
        """返回 session 专属的入站队列，不存在时自动创建（maxsize=64）。"""
        q = self._inbound_queues.get(session_key)
        if q is None:
            q = asyncio.Queue(maxsize=self._inbound_maxsize)
            self._inbound_queues[session_key] = q
        return q
```
...
```python
# core/message_bus.py:122
self._outbound: asyncio.Queue[OutboundMessage] = asyncio.Queue(maxsize=256)
```

- **全局共享**：整个进程只有一个出站队列，所有 session 的所有请求的输出都经过它
- **maxsize=256**：队列满时生产者（`serve()` 中的 `_on_delta` 等回调）被阻塞，形成端到端背压
- **消费者按 `correlation_id` 过滤**：每个消费者读取所有消息，跳过不属于自己的
...
Orchestrator.serve(bus_msg, session_key)                 (orchestrator.py:487-587)
  ...
  while self._running:
      ...
      │                                                      # 1s 超时做 idle check
      │
      ├── 构造流式回调（_on_delta, _on_thinking, _on_tool_start 等）
      │     每个回调 = OutboundMessage(session_key, cid, msg_type, data)
      │              → bus_msg.outbound.put(...)
      │
      ├── result = await self.process_message(
      │       session_key, user_input, ...,
      │       on_delta=_on_delta,           ← 每个 content token → outbound
      │       on_thinking=_on_thinking,     ← 每个 reasoning token → outbound
      │       on_tool_start=_on_tool_start, ← 工具开始 → outbound
      │       ...
      │   )
      │
      └── bus_msg.outbound.put(OutboundMessage(..., "final", {
...
出站方向（Orchestrator → 客户端）:
  outbound queue (maxsize=256) 满 → serve() 中的 _on_delta() 被阻塞
  → LLM token 生成被暂停 → provider 的 stream 迭代被阻塞
  → 端到端流控
```
...
**问题**：当消费者 A 从队列取出消息，发现 `correlation_id` 不匹配而跳过时，该消息已经**丢失**（从队列中移除）。如果消费者 B（本该接收此消息）尚未轮到读取，它将永远收不到该消息。

**影响**：
- 两消费者竞争时，消息可能被错误的消费者丢弃
- 消费者 A 等待 `final` 但被消费者 B 误取了 → 消费者 A 永远挂起
- 消费者 B 收到不匹配的消息后跳过 → 消费者 B 也收不到正确的 `final` → 两条连接都挂起

**缓解因素**：同一 session 内请求**串行处理**（serve 单消费者），所以同一 session 内不会同时有两个活跃请求竞争消息。跨 session 的竞争存在但概率较低——serve task 的 `process_message()` 需要一定时间完成，这期间其他 session 的 serve task 也在并发运行。
...
- **共享 outbound 队列 + correlation_id 过滤**：请求级多路复用，多个消费者各自取自己的消息
- **per-session 写锁**：`SessionManager.lock_session()` 保护数据写入，防止损坏
- **队列容量背压**：`inbound_maxsize=64` + `outbound_maxsize=256` 形成隐式流控
- **WebSocket 请求取消**：支持中断长时间运行的 Agent 操作
- **无全局并发限制**：没有信号量、速率限制器或最大 session 数限制
- **CLI vs HTTP 双模型**：CLI 同步调用（零延迟），HTTP/WS 通过 MessageBus 解耦（支持多用户）
