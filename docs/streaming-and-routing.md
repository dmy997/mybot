...
┌─── HTTP/WS 路径 (orchestrator.py:569-705) ──────────────────────────┐
│                                                                      │
...
Orchestrator ──────►│  outbound("cli")      ──────────►│ CLI (全消费)
                    ...
                    │  outbound("push:{sid}")─────────►│ SSE push_events (不过滤 cid)
                    └──────────────────────────────────┘
```
...
- **push: 频道**：`push:{session_key}` 格式的专项频道，用于定时推送（scheduled tasks）的结果投递。serve() 从 `source="push:<session_key>"` 提取频道名，将输出写入对应的 `push:...` 队列；`GET /events/{session_id}` SSE endpoint 消费该队列，不过滤 `correlation_id`，确保所有推送消息都能到达前端
...
| `new_turn` | `None` | 新回合开始 |
| `hitl_confirm` | `dict` | `{request_id, tool_name, arguments, capabilities}` |
...
4. 入站：bus_msg.inbound("session_abc").put(InboundMessage(
   ...
   │
5. Orchestrator.serve() 后台 task（从步骤 3 开始运行） (orchestrator.py:569-705)
...
6. event_stream() SSE 消费者（步骤 1 返回的 StreamingResponse） (server.py:178-236)
   ...
   │    if out.correlation_id != "a1b2c3d4e5" and out.msg_type != "hitl_confirm": continue  ← 过滤同频道其他请求（HITL 除外）
