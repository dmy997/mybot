...
**阅读顺序：session.py → session_store.py → token_budget.py → compaction.py → context_manager.py → memory_service.py → semantic_filter.py → utils/embedding.py**
...
统一上下文管理器，组合多个子服务：
- `build_messages(images, context_window, max_output_tokens)` — 主入口，构建消息列表
- `_build_system_prompt()` — system prompt 组装（base + skills (三级合并) + tools (语义过滤) + MemoryManager + 冻结快照 + prefetch + file + recent history）
- `_get_semantic_skills(query)` — 基于 embedding 余弦相似度的 skill 排名（top-k=8）
- `_build_static_prompt()` — 静态提示词缓存（语义过滤开启时绕过）
- remember/forget/recall 操作 + session CRUD + TTL 过期
- `_repair_messages()` — 中断会话修复（3 种模式）

### `context/memory_service.py`
会话间记忆服务：冻结快照（per session_key）+ remember/forget/recall + 混合搜索回退。

### `context/semantic_filter.py` (73 行)
语义过滤核心 — embedding 余弦相似度排序：
- `cosine_similarity(a, b)` — 两个等长向量的余弦相似度
- `rank_by_similarity(query, items, top_k, min_score)` — 将 `(name, desc)` 列表与 query 做语义匹配，返回排序后的 `(name, desc, score)` 列表
- Embedding 模型不可用时优雅降级（返回所有项，score=1.0）

### `utils/embedding.py` (76 行)
共享的 `EmbeddingModel` 单例 — 惰性加载 `sentence-transformers`（`all-MiniLM-L6-v2`），384 维向量：
- `encode(texts)` → `list[list[float]] | None` — 批量文本编码
- `available` — 模型是否成功加载
- `get_embedding_model()` — 全局单例获取器

---

## Layer 9: Long-term Memory（长期记忆）

**阅读顺序：provider.py → builtin_provider.py → manager.py → store.py → hybrid_store.py → consolidator.py → dream.py → scrubber.py**

### `memory/provider.py`
`MemoryProvider` (ABC) — 可插拔记忆后端的抽象基类。定义生命周期钩子：`initialize`, `system_prompt_block`, `prefetch`, `sync_turn`, `handle_tool_call`, `shutdown`, `on_session_end`, `on_memory_write`。

### `memory/builtin_provider.py`
`BuiltinMemoryProvider` — 包装现有 MemoryService 的内置提供者，始终活跃。提供 3 个工具 schema（`memory_remember/recall/forget`），`handle_tool_call()` 路由到 MemoryService。

### `memory/manager.py`
`MemoryManager` — 协调内置 + 外部提供者：故障隔离、最多一个外部、工具名路由、Schema 去重。

### `memory/store.py`
`MemoryStore` — 基于文件的统一记忆 I/O（SOUL.md, USER.md, MEMORY.md, history.jsonl + cursor 管理）。
...
- 阶段 2：Dream 周期回顾（CronScheduler 触发，合并和去重记忆）

### `memory/scrubber.py`
上下文围栏清洗：`StreamingContextScrubber` 状态机在流式输出中清洗 `<memory-context>` 标签。

### `plugins/memory/__init__.py`
提供者发现：扫描 `plugins/memory/` 目录查找 `MemoryProvider` 子类。
...
`SkillsLoader` — 基于文件的 Skill 发现（YAML frontmatter），关键词触发器 + 语义相似度自动注入 system prompt。
- `get_triggered_skills(query)` — 大小写不敏感关键词匹配
- `get_always_skills()` — always: true 标记的 skill
- `list_skills()`, `load_skill()`, `load_skills_for_context()`, `build_skills_summary()`
