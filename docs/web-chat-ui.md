...
mybot 提供一个完整的 Web 聊天界面，通过 `mybot-server` 命令启动。后端基于 Starlette（HTTP SSE + WebSocket），前端是单个 HTML 文件（`server_web/index.html`，约 1050 行），使用原生 JavaScript 实现，零构建步骤。页面包含三个可切换视图：聊天、可观测性（Metrics/Log/Trace）。
...
- 最近 200 条结构化日志事件（支持 `?limit=N` 查询参数）
