...
triggers: [前端, frontend, UI, 界面设计, 网页设计]
metadata:
  mybot:
...
| `get_triggered_skills(query)` | `list[str]` | 基于 `triggers` 关键词（大小写不敏感）匹配当前查询的 skill 名称 |
...
line 345:  system_content = await self._build_system_prompt(
           ...
           │
           ├── 合并 skills：explicit + keyword-triggered + semantic top-k（去重）
           ...
           │         (skills or []) + triggered + semantic
           │     ))
           │
           ├── 过滤工具：tools.filter_by_similarity(query, top_k)
           │     # 始终保留 delegate 工具
           │
           └── static = await self._build_static_prompt(tools, all_skills, query=query)
                       # 当 query 非空且语义过滤开启时，绕过静态缓存
...
═══════════════════════════════════════════════════════════════════════════
ContextManager._build_system_prompt() — context_manager.py:289-354
═══════════════════════════════════════════════════════════════════════════
...
`context_manager.py:355-516`

```
_build_static_prompt(tools, skills, query=None)
  │
  ├── 1. 缓存检查（query 为空或语义过滤关闭时使用缓存）
  │     if query and self._semantic_enabled: 跳过缓存
  │     elif self._static_prompt is not None: return self._static_prompt
  │
  ├── 2. 基础系统提示词
  │     parts.append(self.system_prompt or _DEFAULT_SYSTEM_PROMPT)
  │
  ├── 3. 工具语义过滤（P1）
  │     tools = tools.filter_by_similarity(query, top_k=self._semantic_tool_top_k)
  │     # 默认 top_k=None → 不过滤；始终保留 delegate
  │
  ├── 4. Skills 三级组装
  │     │
  │     ├── autoload_skills = self.skills_loader.build_skills_summary(
  │     │       exclude=skill_exclude  # 排除未选中的 skill 以减少冗余
  │     │   )
  │     │
  │     ├── explicit_skills = self.skills_loader.load_skills_for_context(skills or [])
  │     │   # 缺失 skill 显示 "(no skill file found)" 文本
  │     │
...
这种设计确保：
- 系统提示词体积可控（17 个 skill 摘要 ≈ 2KB，而 17 个完整 skill ≈ 50KB+）
...
- **三级合并**：explicit（用户指定）+ keyword-triggered（关键词匹配）+ semantic top-k（语义相似度），`dict.fromkeys()` 去重
- **关键词触发器**：每个 SKILL.md 的 YAML `triggers` 字段支持大小写不敏感的关键词匹配，命中后自动注入上下文
- **语义相似度筛选**：基于 `sentence-transformers` embedding + 余弦相似度，将 skill 描述与用户 query 匹配，取 top-k=8
...
- **always 机制**：通过 `get_always_skills()` 自动注入 `process_message()`，无需用户显式指定
- **静态缓存**：Skill 内容无限期缓存，但 query 非空且语义过滤开启时绕过缓存；运行时新增/修改 skill 需重启生效
