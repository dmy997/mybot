...
├── SessionStore         — 会话持久化（→ SessionManager）
├── MemoryService        — 记忆 CRUD + 混合搜索（→ MemoryStore）
...
`ContextManager.save_exchange()` → `SessionStore.save_exchange()` → `SessionManager`
...
Orchestrator.process_message()
  ...
  │   )
  │
  └── spec = AgentInput(init_messages=messages, ...)
        └── agent.run(spec)
```
...
ContextManager.build_messages()
  │
  ├── 1. _repair_messages(session_messages)
  │     检测 3 种中断模式：未匹配 tool_call / 中断的工具执行 / 未响应的用户输入
  │
  ├── 2. micro_compact(session_messages, ...)
  │     └── CompactionService.micro_compact()
  │           三步规则压缩：清除旧工具结果 / 移除孤立 tool_result / 补全缺失 tool_result
  │
  ├── 3. 过滤历史中的 system 角色消息
  │
  ├── 4. 按 TokenBudget 截断历史工具结果和参数
...
Orchestrator.process_message() 续
  ...
  │     │   委托给 SessionStore.save_exchange()
  │     ├── SessionManager.lock_session(session_key)
  ...
  └── asyncio.create_task(_consolidate_and_prune())
        ...
        │     did = await consolidator.maybe_consolidate(
        ...
        │     │   │     └── 异常降级: store.raw_archive()
        │     │   └── session.last_consolidated += boundary
        │     └── 返回 True/False
        │
        └── if did: SessionManager.prune_archived_messages()
...
```
Orchestrator.serve() 循环
  │
  └── 1s 超时的 idle check
        └── _compress_idle_sessions(session_key)
              │
              └── if idle > idle_compress_seconds: (300s)
                    ctx.compress(session_key)
                      └── CompactionService.auto_compact()
...
Config (config.py)
  ...
  └── BLOCK_BUFFER_RATIO=0.017      → Config.block_buffer_ratio
        │
        ▼
TokenBudget(context_window, max_output_tokens,
  ...
  └── block_threshold = effective_window * (1.0 - block_buffer_ratio)
        │
        ▼
ContextManager(token_budget=TokenBudget(...))
  └── CompactionService(token_budget=...)
```
